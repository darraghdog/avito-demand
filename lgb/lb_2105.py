...
'''
df['pr_usseq_cut'] = pd.qcut(df['price'], 25, duplicates = 'drop').astype(str) + \
                        pd.qcut(df['item_seq_number'], 15, duplicates = 'drop').astype(str)
'''
for col in df.columns:
...
lgsub.to_csv("../sub/lgsub_2105.csv.gz",index=True,header=True, compression = 'gzip')
