...
featnetimg = pd.read_csv(path + '../features/nnetImgStr5CV.csv.gz', compression = 'gzip') # created with features/make/imgnnet2205.py
...
df['nnet_img'] = featnetimg['nnet_imgstr_preds' ].values
df = df.set_index('item_id')
df.drop(['index'], axis=1,inplace=True)

print('RMSE:', np.sqrt(metrics.mean_squared_error(y, df['nnet_img'][:len(traindex)].values)))
...
lgsub.to_csv("../sub/lgsub_2605A.csv.gz",index=True,header=True, compression = 'gzip')
