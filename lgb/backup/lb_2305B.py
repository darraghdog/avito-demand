...
dnimg    = np.vstack([dnimgtrn, dnimgtst])
del dnimgtrn, dnimgtst
gc.collect()

print('[{}] SVD on img features'.format(time.time() - start_time))
SVD = decomposition.TruncatedSVD(n_components=8)
dnimgsvd = SVD.fit_transform(preprocessing.normalize(dnimg, copy=False)).astype(np.float32)
dnimgsvd = csr_matrix(dnimgsvd)
del dnimg
gc.collect()
...
tfvocab = df.columns.tolist() + tfvocab + ['imgsvdcomp%s'%(i) for i in range(dnimgsvd.shape[1])]
...
lgsub.to_csv("../sub/lgsub_2205B.csv.gz",index=True,header=True, compression = 'gzip')
...
'''
[200]   train's rmse: 0.21241   valid's rmse: 0.217202
[400]   train's rmse: 0.204232  valid's rmse: 0.215901
[600]   train's rmse: 0.19887   valid's rmse: 0.215434
....
'''
