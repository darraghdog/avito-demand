...
featct.columns
featgeo = pd.read_csv(path + '../features/user_geo_counts.csv.gz', compression = 'gzip') # created with features/make/user_private_titles.csv.gz
featprvttl = pd.read_csv(path + '../features/user_private_titles.csv.gz', compression = 'gzip') # created with features/make/user_private_titles.csv.gz
featprvttl.rename(columns={'title': 'all_titles'}, inplace = True)
featgeo.head()
featgeo.dtypes
keep = ['user_id', 'user_avg_price', 'user_ad_ct']
df = df.reset_index().merge(featusrttl[keep], on = 'user_id', how = 'left').set_index('item_id')
df = df.reset_index().merge(featprvttl, on = 'user_id', how = 'left').set_index('item_id')
df = df.reset_index().merge(featgeo, on = 'user_id', how = 'left').set_index('item_id')
df['all_titles'].fillna('', inplace = True)
df.isnull().sum()
df.head()
...
plt.savefig(path + '../plots/feature_import_2605.png')
...
lgsub.to_csv("../sub/lgsub_2605.csv.gz",index=True,header=True, compression = 'gzip')
