...
for f in range(6):
    ...
    del X_test
    gc.collect()
    y_pred_trn.to_csv("lgbCV_1406A_trn.csv",index=True)
    y_pred_tst.to_csv("lgbCV_1406A_tst.csv",index=True)    
...
# RMSE for all : 0.2168
lgsub.to_csv("lgCV_1406A.csv.gz",index=True,header=True, compression = 'gzip')

lgsub.to_csv(path + "../sub/lgCV_1406A.csv.gz",index=True,header=True, compression = 'gzip')
