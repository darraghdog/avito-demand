...
featcatvar = pd.read_csv(path + '../features/price_variance_categories.gz', compression = 'gzip') # created with features/make/user_actagg_1705.py
...
df.head()
df = pd.concat([df.reset_index(),featenc, featct, featrdgtxt, featrdgprc, featcatvar],axis=1)
...
lgsub.to_csv("../sub/lgsub_2505.csv.gz",index=True,header=True, compression = 'gzip')
