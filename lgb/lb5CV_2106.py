...
featlast5  = pd.read_csv(path + '../features/last_five_title_2106.gz', compression = 'gzip', sep = '\t')
...
df.head()
df = pd.concat([df.reset_index(),featenc, featct, featrdgtxt, featrdgprc, featimgprc, featrdgrnk, featnumf, featprmenc, featprmtro, featencfst, featprtfst, featldlag, featlast5],axis=1)
...
df.columns
del featusrttl, featusrcat, featusrprd, featenc, featrdgprc, featimgprc, featnumf, featprmenc, featprmtro, featencfst, featprtfst, featldlag, featlast5
...
df['titles_last_five'] = parallelize(df['titles_last_five'], tokCol)
...
vectorizer = FeatureUnion([
        ...
        ('titles_last_five',TfidfVectorizer(
        ngram_range=(1, 2),
        max_features=150000,
        **tfidf_para,
        preprocessor=get_col('titles_last_five'))),
    ])
