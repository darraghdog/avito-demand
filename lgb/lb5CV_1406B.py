...
featminmax = pd.read_csv(path + '../features/minmax_fest_1206.gz', compression = 'gzip')
...
df.head()
df = pd.concat([df.reset_index(),featenc, featct, featrdgtxt, featrdgprc, featimgprc, featrdgrnk, featnumf, featprmenc, featprmtro, featencfst, featprtfst, featminmax ],axis=1)
...
df.columns
del featusrttl, featusrcat, featusrprd, featenc, featrdgprc, featimgprc, featnumf, featprmenc, featprmtro, featencfst, featprtfst, featminmax
...
for f in range(6):
    ...
    del X_test
    gc.collect()
    y_pred_trn.to_csv("lgbCV_1406B_trn.csv",index=True)
    y_pred_tst.to_csv("lgbCV_1406B_tst.csv",index=True)    
...
# RMSE for all : 0.2168
lgsub.to_csv("lgCV_1406B.csv.gz",index=True,header=True, compression = 'gzip')

lgsub.to_csv(path + "../sub/lgCV_1406B.csv.gz",index=True,header=True, compression = 'gzip')
