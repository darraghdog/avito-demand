...
print('[{}] Load translated engineered features'.format(time.time() - start_time))
translatedf = pd.read_csv(path + '../features/translate_trn_en.csv.gz')
translatedf.columns
translatedf = translatedf[['item_id', 'param_1_translated', 'param_2_translated', \
       'param_3_translated', 'title_translated', 'description_translated']].set_index('item_id')
appenddf = traindf[(traindf.activation_date<=pd.to_datetime('2017-03-26')).values]
appenddf.shape
appenddf = appenddf.merge(translatedf, left_index=True, right_index=True, how = 'left')
for col in ['param_1_translated', 'param_2_translated', 'param_3_translated', 'title_translated', 'description_translated']:
    print('Replace %s'%(col))
    col_to, col_from = col.replace('_translated', ''), col
    appenddf[col_to] = appenddf[col_from]
    appenddf.drop(col_from, inplace = True, axis = 1)
appenddf.reset_index(inplace=True)
appenddf['item_id'] = appenddf['item_id'] + ['_app']
appenddf = appenddf.set_index('item_id')
traindf = pd.concat([traindf, appenddf], axis = 0)
gc.collect()


print('[{}] Load Rest of Train/Test'.format(time.time() - start_time))
...
translatedf = translatedf[['item_id', 'param_1_translated', 'param_2_translated', \
       'param_3_translated', 'title_translated', 'description_translated']].set_index('item_id')
translatedf.shape
appenddf = df.loc[traindex,:][trnidx]
appenddf.shape
appenddf = appenddf.merge(translatedf, left_index=True, right_index=True, how = 'left')
col = 'param_1_translated'
for col in ['param_1_translated', 'param_2_translated', 'param_3_translated', 'title_translated', 'description_translated']:
    print('Replace %s'%(col))
    col_to, col_from = col.replace('_translated', ''), col
    appenddf[col_to] = appenddf[col_from]
...
if load_text:
    dftxt = pd.read_csv(path + '../features/text_features_morphed_translated.csv.gz', compression = 'gzip')
    ...
    df[text_cols].to_csv(path + '../features/text_features_morphed_translated.csv.gz', compression = 'gzip')
gc.collect()
