...
vectorizer.fit(pd.concat([df[text_cols].loc[traindex,:], dftrl[text_cols].loc[traindex,:]]).to_dict('records'))
ready_df    = vectorizer.transform(df.to_dict('records'))
ready_dftrl = vectorizer.transform(dftrl.to_dict('records'))
...
X_traintrl = hstack([csr_matrix(df.loc[traindex,:][trnidx].values),ready_dftrl[0:traindex.shape[0]][trnidx]])
...
lgtrain = lgb.Dataset(vstack([X_train, X_traintrl]), pd.concat([y_train, y_train]),
                feature_name=tfvocab)
