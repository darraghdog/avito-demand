...
for f in range(6):
    ...
    del X_test
    gc.collect()
    y_pred_trn.to_csv("lgbCV_0906_trn.csv",index=True)
    y_pred_tst.to_csv("lgbCV_0906_tst.csv",index=True)    
...
# RMSE for all : 0.2168
lgsub.to_csv("lgCV_0906.csv.gz",index=True,header=True, compression = 'gzip')

lgsub.to_csv(path + "../sub/lgCV_0906.csv.gz",index=True,header=True, compression = 'gzip')
