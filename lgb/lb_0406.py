...
featenc2   = pd.read_csv(path + '../features/alldf_bayes_mean0406.gz', compression = 'gzip') # created with features/make/user_actagg_1705.py
...
df.head()
df = pd.concat([df.reset_index(),featenc, featenc2, featct, featrdgtxt, featrdgprc, featimgprc],axis=1)
...
df.columns
del featusrttl, featusrcat, featusrprd, featenc, featrdgprc, featimgprc, featenc2
...
plt.savefig(path + '../plots/feature_import_0406.png')
