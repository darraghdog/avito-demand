...
featcatrto = pd.read_csv(path + '../features/pcat_ratios_fest_1806.gz', compression = 'gzip')
#featimgnet.drop('item_id', axis = 1, inplace = True) 
...
df.head()
df = pd.concat([df.reset_index(),featenc, featct, featrdgtxt, featrdgprc, featimgprc, featrdgrnk, featnumf, featprmenc, featprmtro, featencfst, featprtfst, featcatrto],axis=1)
...
for bag in range(bags):
    ...
    for f in range(6):
        ...
        if f == 5:
            ...
            print('RMSE:', np.sqrt(metrics.mean_squared_error(y_test, y_pred_trn[~trnidx] / (bag+1))))
        del X_test
        gc.collect()
        y_pred_trn.to_csv("lgbCV_1806B_trn.csv",index=True)
        y_pred_tst.to_csv("lgbCV_1806B_tst.csv",index=True)    
...
# RMSE for all : 0.2168
lgsub.to_csv("lgCV_1806B.csv.gz",index=True,header=True, compression = 'gzip')

lgsub.to_csv(path + "../sub/lgCV_1806B.csv.gz",index=True,header=True, compression = 'gzip')
