...
for bag in range(bags):
    ...
    for f in range(6):
        ...
        if f == 5:
            ...
            X_test = hstack([csr_matrix(tstdf),ready_df[traindex.shape[0]:]])
            ...
            tstdf[tstdf<0] = 0
            tstdf = scaler.transform(tstdf.values)
            X_test = hstack([csr_matrix(tstdf), ready_df[0:traindex.shape[0]][~trnidx]])
        ...
        del X_test
        gc.collect()
        y_pred_trn.to_csv("rdg5CV_2306B_trn.csv",index=True)
        y_pred_tst.to_csv("rdg5CV_2306B_tst.csv",index=True)    
...
# RMSE for all : 0.2168
lgsub.to_csv("rdg5CV_2306B.csv.gz",index=True,header=True, compression = 'gzip')

lgsub.to_csv(path + "../sub/rdg5CV_2306B.csv.gz",index=True,header=True, compression = 'gzip')
