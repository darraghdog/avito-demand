...
y_pred_trn = pd.read_pickle('y_pred_trn0.pkl')
y_pred_tst = pd.read_pickle('y_pred_tst0.pkl')
for f in range(6)[1:]:
    ...
    gc.collect()
    y_pred_trn.to_pickle('y_pred_trn.pkl')
    y_pred_tst.to_pickle('y_pred_tst.pkl')
...
# RMSE for all : 0.2168
rnnsub.to_csv("rnndhCV_2705A.csv.gz",index=True,header=True, compression = 'gzip')
