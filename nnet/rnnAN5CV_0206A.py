...
foldls = [["2017-03-15", "2017-03-16", "2017-03-17"],        ["2017-03-18", "2017-03-19", "2017-03-20"],        ["2017-03-21", "2017-03-22", "2017-03-23"],        ["2017-03-24", "2017-03-25", "2017-03-26"],         ["2017-03-27", "2017-03-28", "2017-03-29",             "2017-03-30", "2017-03-31", "2017-04-01",             "2017-04-02", "2017-04-03","2017-04-07"]]
...
embed_me = ["emb_item_seq_number", "user_id","image_top_1", "region", 'city', 'emb_price',             "parent_category_name", "category_name", "user_type", "emb_weekday", 'text_feat']             + ['cat_param_'+str(i+1) for i in range(3)]
for col in embed_me:
...
#Get the file  https://github.com/facebookresearch/fastText/blob/master/docs/crawl-vectors.md
#Get the file  http://panchenko.me/data/dsl-backup/w2v-ru/
from tqdm import tqdm
preembsz = 100
word_vectors = KeyedVectors.load_word2vec_format(path+ '../features/all.norm-sz100-w10-cb0-it1-min100.w2v', binary=False)
weights= np.random.uniform(-.5,.5,MAX_DSC*preembsz).reshape(MAX_DSC,preembsz)
for word, index in tqdm(tok_raw.items()):
...
def get_model(emb_size = 32, dr = 0.1, l2_val = 0.0001):
    ...
    #Embeddings layers
    embs_text = Embedding(MAX_DSC, preembsz, embeddings_regularizer=l2(l2_val), name='text_emb', 
...
def train_model(dtrain, dvalid):  
    ...
    '''
    y_sub = to_proba(sum([sum(to_logit(y_pred_ls))/len(y_pred_ls) for bag in range(bags)])/bags)     
...
for f in range(6):
    ...
    df[['rnn_preds']].to_csv('../sub/rnnCV_0206A_tmp.csv.gz', index=True, header=True, compression='gzip')
    gc.collect()
...
df[['rnn_preds']].to_csv('../sub/rnnCV_0206A.csv.gz', index=True, header=True, compression='gzip')
