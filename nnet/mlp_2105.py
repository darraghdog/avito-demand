...
def fit_predict(xs, y_train) -> np.ndarray:
    ...
    with tf.Session(graph=tf.Graph(), config=config) as sess, timer('fit_predict'):
        ...
        out = ks.layers.Dense(192, activation='relu', kernel_regularizer=l2(0.0001))(model_in)
        #out = ks.layers.Dense(512, activation='relu')(model_in)
        out = ks.layers.Dense(64, activation='relu', kernel_regularizer=l2(0.0001))(out)
        out = ks.layers.Dense(64, activation='relu', kernel_regularizer=l2(0.0001))(out)
...
# baseline : Valid RMSE: 0.2162
        
# baseline 1 epoch with .001 -- 0.2246
# baseline 3 epoch with .001 -- 0.2218       
# baseline 3 epoch with .003 -- 0.2259      
