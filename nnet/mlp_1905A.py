...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['image_top_1_cut'] = 'imt1_'+(np.log1p(df['item_seq_number'])*5).fillna(150).astype(np.int32).astype(str)
    ...
    return df[['name', 'text', 'user', 'region', 'city', 'item_seq_number_cut', 'all_titles', 'user_avg_price_cut'\
               ...
               , 'usercat_avg_price_cut', 'usercat_ad_ct', 'image_top_1_cut']]
...
if __name__ == '__main__':
    ...
    if full: 
        ...
        mlpsub.to_csv("../sub/mlpsub_1905A.csv.gz",index=True,header=True, compression = 'gzip')
        print("All done")
        ...
        blend_lb.to_csv("../sub/mlpblend_1905A.csv.gz",index=False,header=True, compression = 'gzip')
