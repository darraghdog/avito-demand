...
rgx = re.compile('[%s]' % '!"#%&()*,-./:;<=>?@[\\]^_`{|}~\t\n')   
df['description']    =df['description'].str.lower().map(tokSent)
df['title']          =df['title'].str.lower().map(tokSent)
...
#This code grabs all of the
res = np.full((epochs,epochs+1),1.)
for i in range(epochs):
    for j in range(i+1,epochs+1):
        preds = sum([sum(y_pred_ls[i+epochs*bag:j+epochs*bag])/len(y_pred_ls[i+epochs*bag:j+epochs*bag]) for bag in range(bags)])/bags
        res[i,j] = np.sqrt(metrics.mean_squared_error(dvalid['target'], preds.flatten()))
...
rnnsub.to_csv("../../sub/rnnsub_2205.csv.gz",index=True,header=True, compression = 'gzip')
