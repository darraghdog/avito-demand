...
def load_data(full):
    ...
    featusrttl = pd.read_csv(path + '../features/user_titles.csv.gz', compression = 'gzip') # created with features/make/user_titles_1605.py
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df[['name', 'text', 'user', 'region', 'city', 'item_seq_number_cut', 'all_titles' \
...
if __name__ == '__main__':
    ...
    if full: 
        ...
        mlpsub.to_csv("../sub/mlpsub_1605B.csv.gz",index=True,header=True, compression = 'gzip')
        print("All done")
        ...
        blend_lb.to_csv("../sub/mlpblend_1605B.csv.gz",index=False,header=True, compression = 'gzip')
