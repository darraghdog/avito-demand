...
def load_data(full):
    ...
    pcols = ['pcat_log_itseq_rratio', 'cat_price_rratio', 'ttl_price_rratio']
    featrdgprc = featrdgprc[pcols]
    for col in pcols:
        featrdgprc[col].fillna(-1, inplace = True)
        featrdgprc[col] = pd.qcut(featrdgprc[col], 40, duplicates = 'drop', retbins = False, labels = False).values
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df[['name', 'text', 'user', 'region', 'city', 'item_seq_number_cut', 'all_titles', 'user_avg_price_cut'\
               ...
               , 'usercat_avg_price_cut', 'usercat_ad_ct']+pcols]
...
def main(full = False):
    vectorizer = make_union(
        ...
        on_field(['region', 'city', 'price_cut', 'item_seq_number_cut', 'image_top_1', 'user_avg_price_cut', \
                  'param_1', 'param_3', 'param_3', 'user_type', 'user', 'user_ad_ct', 'usercat_avg_price_cut', 
                  'usercat_ad_ct', 'pcat_log_itseq_rratio', 'cat_price_rratio', 'ttl_price_rratio'],
...
# [fit_predict] done in 1262 s ; Valid RMSE: 0.2162