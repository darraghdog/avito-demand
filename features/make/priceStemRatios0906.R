...
keepcols = c("title","parent_category_name", "category_name", "price", 'region', 'city',
...
alladf[, index := -1]
stemdfsm = data.table(read_csv(paste0(path, '../features/title_stem.csv')))
alldfsm[, title_stem := stemdfsm$title_stem]
stemdf = data.table(read_csv(paste0(path, '../features/title_stem_active.csv')))
alladf[, title_stem := stemdf$title_stem]
rm(stemdf, stemdfsm)
alldfsm[,.(title, title_stem)]
alladf[,.(title, title_stem)]
gc(); gc(); gc(); gc(); gc(); gc()
alldf = rbind(alladf, alldfsm)
rm(alladf, alldfsm)
gc(); gc(); gc(); gc(); gc(); gc()
table(alldf[,.N, by = title][order(N)]$N)
length(unique(alldf$title_stem))
length(unique(alldf$title))
