...
keepcols = c("parent_category_name", "category_name", "price",  'region', 'city', 'user_type', 'activation_date', 'user_id',
...
trndf[,is_train:=1]
gc(); gc()
tstdf = data.table(read_csv(paste0(path, 'test.csv'), col_types = list( 'item_id' = col_skip())))
...
encodeDF = function(alldf, col, prior = 1000){
  ...
  setnames(alldf, "tmpcol", paste0(paste0(cols, collapse = '_'), "_enc4"))
  return(alldf)
}

priceRatios = function(df, cols, prior){
  df[, `:=`(ct = .N, meanpr = mean(price, na.rm = T)), 
        by = cols]
  df[, tmpcol:= (((price/meanpr)*ct)+(20))/(ct+20)]
  df[is.na(tmpcol), tmpcol:= 1]
  setnames(alldf, "tmpcol", paste0(paste0(cols, collapse = '_'), "_fratio4"))
  return(alldf)
}
...
for (l in ls){
  ...
  setnames(alldf, "tmpct", paste0(paste0(cols, collapse = '_'), "_count4"))
}

#Create a second title column
alldf_enc = alldf[, grep('_enc4|_count4|_fratio4', colnames(alldf), value = T), with = F]
View(alldf_enc[1:10000])
sum(is.na(alldf_enc))

# Remove very low occurence ones
lenunq = function(x) length(unique(x))
ls = lapply(alldf_enc, lenunq)
table(ls>20)
alldf_enc = alldf_enc[, ls>20, with = F]
...
writeme(alldf_enc, 'feature_fest_1206')
rm(list=ls())
gc();gc();gc()
