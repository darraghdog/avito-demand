...
encodeDF = function(alldf, col, prior = 1000){
  ...
  setnames(alldf, "tmpcol", paste0(col, "_enc3", collapse = '_'))
  return(alldf)
}
...
alldf = encodeDF(alldf, c("category_name", 'param_1'), prior = 50)
alldf = encodeDF(alldf, c("category_name", 'param_1', 'region', 'city'), prior = 50)
alldf = encodeDF(alldf, c("category_name", 'param_1', 'user_type'), prior = 50)
alldf = encodeDF(alldf, c("category_name", 'param_1', 'param_2'), prior = 50)
alldf = encodeDF(alldf, c("category_name", 'param_1', 'param_2', 'region', 'city'), prior = 50)
alldf = encodeDF(alldf, c("category_name", 'param_1', 'param_2', 'user_type'), prior = 50)
alldf_enc = alldf[, grep('_enc3', colnames(alldf), value = T), with = F]
...
writeme(alldf_enc, 'alldf_bayes_mean_param_1006')
rm(list=ls())
gc();gc();gc()
