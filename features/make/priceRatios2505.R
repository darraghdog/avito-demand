...
alldf[, `:=`(ct = .N, meanpr = mean(price, na.rm = T), meansq = mean(item_seq_number, na.rm = T)), 
      by = .(parent_category_name, category_name, title, description)]
alldf[, dscr_price_rratio := (((price/meanpr)*ct)+(5))/(ct+5)]
#alldf[, dscr_itseq_rratio := (((item_seq_number/meansq)*ct)+(5))/(ct+5)]
...
writeme(alldfout[,grep('_rratio', colnames(alldfout), value = T), with = F], 'price_seq_category_ratios')
rm(list=ls())
gc();gc();gc()