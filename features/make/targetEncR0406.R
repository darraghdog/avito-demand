...
keepcols = c("user_id", "category_name", 'title', "image_top_1", 'activation_date', 'deal_probability')
...
encodeDF = function(alldf, col, prior = 1000){
  ...
  setnames(alldf, "tmpcol", paste0(col, "_enc2", collapse = '_'))
  return(alldf)
}


#Create a second title column
alldf[, title2 := gsub('[[:punct:] ]+',' ',tolower(title))]


alldf = encodeDF(alldf, c("category_name", 'image_top_1'), prior = 100)
alldf = encodeDF(alldf, c('title'), prior = 15)
alldf = encodeDF(alldf, c('title2'), prior = 15)
alldf_enc = alldf[, grep('_enc2', colnames(alldf), value = T), with = F]
...
writeme(alldf_enc, 'alldf_bayes_mean0406')
rm(list=ls())
gc();gc();gc()
