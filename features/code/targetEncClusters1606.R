...
clusts = fread(paste0(path, '../features/title_clusters.csv'))
...
for(clustcol in c("cluster100", "cluster50")){
  print(clustcol)
  for (l in ls){
    cols = c(clustcol, l[[1]])
    prior = l[[2]]
...
alldf_enc = alldf[, grep('clust', colnames(alldf), value = T), with = F]
alldf_enc[, `:=`(cluster50=NULL, cluster100=NULL) ]
colnames(alldf_enc)
...
writeme(alldf_enc, 'clust_enc_1606')
rm(list=ls())
gc();gc();gc()
