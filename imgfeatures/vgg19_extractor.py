...
end_shape = 4096 #  model.layers[-1].output_shape[-1]
#print('End Shape : %s'(end_shape))
...
for file_ in ['test_jpg', 'train_jpg']: # ,'test_jpg', 
    ...
    gc.collect()
    fnamemat = path + '../features/vgg19_pool_mat_%s'%(file_)
    fnamefls = path + '../features/vgg19_pool_fls_%s'%(file_)
...
for file_ in ['test', 'train']: # ,'test_jpg',
    file_
    fnamemat = path + '../features/vgg19_pool_mat_%s_jpg.npy'%(file_)
    fnamefls = path + '../features/vgg19_pool_fls_%s_jpg'%(file_)
    ...
    #allmat = allmat.tocsr()
    fname = path + '../features/vgg19_pool_array_%s'%(file_)
